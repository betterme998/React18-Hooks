...
11 月 17 日  
问题：  
快速点击时你没有及时调用上一项的 deactivate，或者 deactivate 被延迟到 wait/切换之后，所以上一个 NavIcon 的 isActive 仍为 true，poster 显示为已选中状态。解决办法：在等待动画或切换之前立即 deactivate 上一个激活项，然后再等待当前点击项的过渡，最后 setActiveKey 并 activate 新项。

11 月 19 日  
问题：  
快速点击视频标签时，未选中的标签封面属性会变成选中状态。  
原因。调用 load() 强制恢复到 poster（避免显示最后一帧）

解决：

实现：  
点击非激活标签时仅在过渡（缩小/放大）完成后才真正切换并 activate 新项；  
在等待期间如果快速点回原来的激活项，取消待切换，不改变已激活项；  
点击非激活项时不立即 deactivate 当前激活项（避免旧封面/视频被提前替换）。
