...
■ 目前我们已经主要学习了三种状态管理方式
口 方式一:组件中自己的 state 管理
口 方式二:Context 数据的共享状态
口 方式三:Redux 管理应用状态

■ 在开发中如何选择呢?
口 首先，这个没有一个标准的答案，
口 某些用户，选择将所有的状态放到 redux 中进行管理，因为这样方便追踪和共享;
口 有些用户，选择将某些组件自己的状态放到组件内部进行管理;
口 有些用户，将类似于主题、用户信息等数据放到 Context 中进行共享和管理;
口做一个开发者，到底选择怎样的状态管理方式，是你的工作之一，可以一个最好的平衡方式(Find abalance that worksfor you, and go with it.);

■ 目前项目中我采用的 state 管理方案:
口 UI 相关的组件内部可以维护的状态，在组件内部自己来维护:
口 大部分需要共享的状态，都交给 redux 来管理和维护
口 从服务器请求的数据(包括请求的操作)，交给 redux 来维护

# 4.4.React Router.x 路由
